function status = feature_loop_si_hv(test_structs, clip_structs, vert_size, horiz_size, ...
...
% SYNTAX
%  feature_loop_si_hv(test_structs, clip_structs,vert_size, horiz_size,tslice_length_sec,feature_base_dir)
%  feature_loop_si_hv(...,'PropertyName',...);
% DESCRIPTION
    ...
    try,
        ...
        for cnt = 1:number_tslices,
            ...
            if row*col*time*8/1e6 < 200,
                ...
                y = single(y);
                pack;
                si = zeros(row-12,col-12,time,'single');
                hv = zeros(row-12,col-12,time,'single');
                hvb = zeros(row-12,col-12,time,'single');
                for img = 1:time,
