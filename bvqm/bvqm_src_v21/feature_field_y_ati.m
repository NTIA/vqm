function status = feature_field_y_ati(test_structs, clip_structs,...
    feature_base_dir, varargin)
% FEATURE_FIELD_Y_ATI
%  Loop through a list of clips, compute field-based metrics used by the
%  sequence-based temporal registration algorithm, and save each clip's
%  features in a separate file. 
% SYNTAX
%  feature_field_y_ati(test_structs, clip_structs,feature_base_dir)
%  feature_field_y_ati(...,'PropertyName',...);
% DESCRIPTION
...
feature_name_ti4 = standard_feature_names('Y', 'fullfield',...
        'block_stat','rms', 'specific','cont', 'specific','ti4');
...
for loop = 1:max(size(clip_structs)),
    try,
        ...
        [name_exists_3] = write_feature_standard(0,feature_base_dir,feature_name_ti4,clip_structs(loop),'exists');
        [name_exists_4] = write_feature_standard(0,feature_base_dir,feature_name_ti10,clip_structs(loop),'exists');
        if name_exists_1 && name_exists_2 && name_exists_3 && name_exists_4,
            if verbose,
        ...
        if ~strcmp(clip_structs(loop).video_standard,'progressive'),
            ...
            for cnt = 1:number_tslices,
                ...
                if cnt >= 3,
                    ...
                    [data_ti4(:,:,cnt*2-5)]  = block_statistic (one - buffer(:,:,use), rows, cols, 'rms');
                    [data_ti4(:,:,cnt*2-4)]  = block_statistic (two - buffer(:,:,use+1), rows, cols, 'rms');
                end
            ...
            for cnt = 1:number_tslices,
                ...
                if cnt >= 3,
                    ...
                    [data_ti4(:,:,cnt)]  = block_statistic (y - buffer(:,:,use), rows, cols, 'rms');
                end
        ...
        write_feature_standard(data_ti4, feature_base_dir,feature_name_ti4, clip_structs(loop));
        ...
        clear data_y_mean data_ti2 data_ti4 data_t10;
    catch
        status = 1;
        if verbose,
            fprintf('\n%s\n', lasterr);
