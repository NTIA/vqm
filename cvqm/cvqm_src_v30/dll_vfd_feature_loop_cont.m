function [data, datao, data2, data2o, status] = dll_vfd_feature_loop_cont(...
...
%  frame delay (VFD) contrast, and output the processed clip's features.
% SYNTAX
%  [data, datao, data2, data2o, status] = dll_vfd_feature_loop_cont(...
%        clip_structs, deg_size, time_size, viewing_distance);
% DESCRIPTION
...
%  Dimension output arrays as needed
optional_mean = 1;
datao = zeros(nvert,nhoriz,nslices);
data2o = zeros(nvert,nhoriz,nslices);
...
for cnt = 1:nslices
    ...
    if (~optional_mean)
        datao(:,:,cnt) = block_statistic (Y_GLOBAL(:,:,beg_frame:end_frame), vsize, hsize, 'std');
    else
        [datao(:,:,cnt),data2o(:,:,cnt)] = block_statistic (Y_GLOBAL(:,:,beg_frame:end_frame), vsize, hsize, 'std','mean');
    end
